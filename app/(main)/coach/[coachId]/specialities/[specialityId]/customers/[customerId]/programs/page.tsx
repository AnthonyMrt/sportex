...
import CoachProgramCard from "../_components/CoachProgrammCard";
            ...
            <div className="grid sm:grid-cols-2  lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {customerDetails.Program.map(item => (
                <CoachProgramCard
                  ...
                  price={item.price}
                  progress={
                    item.ProgramGoals.length > 0
                      ? Math.round(
                          (item.ProgramGoals.filter(
                            goal => goal.status === STATUS.DONE
                          ).length /
                            item.ProgramGoals.length) *
                            100
                        )
                      : 0
                  }
