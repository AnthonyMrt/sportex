...
export const getAllCustomers = async () => {
  try {
    const user = await getCurrentUser();
    if (!user || !user.currentSpeciality || !user.Coach)
...
export const deleteGoalForUser = async (goalId: string) => {
  try {
    const user = await currentUser();
    if (!user) throw new Error("Access denied");

    const goalDelete = db.programGoals.delete({
      where: {
        id: goalId,
      },
    });
    ...
    throw new Error("Error in deleteGoal");
  }
};
...
export const createProgramGoal = async (data: {
  description: string;
  start: Date;
  end: Date;
  programId: string;
...
export const updateProgramGoal = async (data: {
  description: string;
  start: Date;
  end: Date;
  goalId: string;
...
export const updateCurrentSpeciality = async (specialityId: string) => {
  try {
    const user = await currentUser();
    if (!user) throw new Error("Access denied!");
...
export const getRevenue = async () => {
  try {
    ...
    const revenue = currentSpeciality.Customer.reduce((acc, customer) => {
      return (
        acc +
        customer.Program.reduce((acc, program) => {
